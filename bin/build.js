...
class Git {
  ...
  async checkStatus() {
    ...
    await Promise.all(primoseArr).then(() => {
      console.log(
        chalk.rgb(13, 188, 121)("全部模块分支代码状态没有未提交代码。")
      );
    });
  }
  ...
  async checkoutBranch() {
    ...
    let branch = "";
    console.log(
      chalk.rgb(17, 168, 203)(`所有模块切换到${this.info.branch}分支`)
    );
            ...
            getStdout: (stdout) => {
              ...
              console.log(
                chalk.rgb(
                  ...
                  168,
                  203
                )(`当前分支为:${branch} ————> 切换到${this.info.branch}分支`)
              );
  ...
  async pullCode() {
    ...
    const reg = /(?<=\*\s)(.+?)(?=\n)/g; // [] 中括号
    console.log(
      chalk.rgb(17, 168, 203)(`更新所有模块${this.info.branch}分支代码`)
    );
  ...
  async codeCompression() {
    console.log(
      chalk.rgb(
        ...
        168,
        203
      )(`开始将dit目录文件，打包成${this.zipName}.zip包文件`)
    );
    this.spinner = ora(
      chalk.rgb(17, 168, 203)(`在打包成${this.zipName}.zip包中请稍后.....\n`)
    );
    this.spinner.start();
    ...
    const output = fs.createWriteStream(
      path.join(__dirname, `../zip/${this.zipName}.zip`)
    ); // 将压缩包保存到当前项目的目录下，并且压缩包名为this.zipName.zip
